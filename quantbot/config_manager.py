...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    #
    # 사용자가 거래소 앱에서 직접 매수하면 봇은 그 사실을 모릅니다. 그대로 두면
    # 같은 종목을 또 사서 의도보다 큰 포지션이 됩니다. 이 값을 주면 주기적으로
    # 잔고를 읽어 봇 상태를 맞춥니다. 텔레그램 /동기화 로 즉시 실행할 수도 있습니다.
    #
    # 빗썸은 주문 이력 조회 API가 없어 기존 주문 대사가 동작하지 않으므로,
    # **잔고를 기준으로** 판단합니다. 보유가 확인되면 추가 매수를 막을 뿐이고,
    # 보유가 없다고 해서 매수를 열어주지는 않습니다(직접 판 것을 되사면 곤란하므로).
    "position_sync_min": 30,
