...
| 방식 | 계산 | 성격 |
|---|---|---|
| `equal` (기본) | 종목당 자본의 1/N 균등 | 수익률 우선 |
| `atr` | 총자산 x 리스크 / (2N) | **낙폭 통제 우선** |

`atr`는 **변동성이 큰 종목일수록 적게 삽니다.** 손절폭(2N)만큼 불리하게 움직였을 때
잃는 금액이 총자산의 `risk_per_trade`가 되도록 수량을 맞춥니다.

포트폴리오 백테스트 결과(워크포워드 10개 구간, OOS 중앙값)입니다.
...
**수익률을 내주고 낙폭을 사는 거래**입니다. 굴리는 금액이 커져 낙폭이 부담될 때
쓰시면 됩니다. 재현: `python -m tools.backtest_portfolio`

> [!TIP]
> 두 방식을 비교하려면 폴더를 두 개 만들어 각각 다른 `config.json`을 두세요.
> 한쪽은 `"force_simulation": true`, `"telegram_enabled": false`로 두면
> 실전과 나란히 돌리며 관찰할 수 있습니다. (아래 "데이터 저장 위치" 참고)
