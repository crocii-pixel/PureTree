...
class CPureTree {
public:
  using OnNodeValueChanged =
      std::function<void(hTree nodeId, const AnyValue &newValue)>;
  OnNodeValueChanged onValueChanged = nullptr;

  explicit CPureTree(const TreeConfig &config = TreeConfig())
      : config_(config) {
    nodes_.reserve(1024);
    links_.reserve(2048);
  }
  ...
  virtual hTree addNode(int type, const std::string &key,
  ...
  virtual hTree addLink(hTree from, hTree to, int type = 0,
                        Metrics metrics = Metrics()) {
    if (!nodes_.exists(from) || !nodes_.exists(to))
      return 0;
  ...
  virtual void removeNode(hTree nodeId) {
    ...
    linksToDelete.insert(linksToDelete.end(), node->outs.begin(),
                         node->outs.end());
    linksToDelete.insert(linksToDelete.end(), node->ins.begin(),
                         node->ins.end());

    std::sort(linksToDelete.begin(), linksToDelete.end());
    linksToDelete.erase(std::unique(linksToDelete.begin(), linksToDelete.end()),
                        linksToDelete.end());
  ...
  virtual void setNodeValue(hTree nodeId, AnyValue newVal) {
    ...
    if (onValueChanged)
      onValueChanged(nodeId, node->value);
  }
  ...
  void serialize(hTree root, std::vector<hTree> &outResult,
                 bool postOrder = true,
                 FilterMode filterMode = FilterMode::None,
                 const std::vector<bool> &filterMask = {}) {
    outResult.clear();
    if (!nodes_.exists(root))
      return;
    ...
    if (filterMask.empty()) {
      if (filterMode == FilterMode::Skip) {
        ...
        // If "Only" is specified but no types are provided, it means NO links
        // can be traversed.
    ...
    traverse(root, outResult, postOrder, filterMode, filterMask);
  }

  std::vector<hTree> serialize(hTree root, bool postOrder = true,
                               FilterMode filterMode = FilterMode::None,
                               const std::vector<bool> &filterMask = {}) {
    std::vector<hTree> result;
    serialize(root, result, postOrder, filterMode, filterMask);
    return result;
  }
...
private:
  void traverse(hTree current, std::vector<hTree> &result, bool postOrder,
                FilterMode filterMode, const std::vector<bool> &filterMask) {
    Node *node = nodes_.getById(current);
    if (!node)
      return;

    if (node->visitEpoch == m_currentTraversalEpoch)
      return;
    ...
    for (hTree linkId : node->outs) {
      ...
      if (filterMode != FilterMode::None && !filterMask.empty()) {
        bool typeMatch = (link->type >= 0 &&
                          static_cast<size_t>(link->type) < filterMask.size())
                             ? filterMask[link->type]
                             : false;

        if (filterMode == FilterMode::Skip && typeMatch)
          continue;
        if (filterMode == FilterMode::Only && !typeMatch)
          continue;
      }

      traverse(link->to, result, postOrder, filterMode, filterMask);
    }
